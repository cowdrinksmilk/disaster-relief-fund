...
//Definitely not "borrowed" from Heroes API assignment
...
public class CupboardFileDAO implements CupboardDAO {
    ...
    private static final Logger LOG = Logger.getLogger(CupboardFileDAO.class.getName()); // Added logging
    ...
    public CupboardFileDAO(@Value("${needs.file}") String filename, ObjectMapper objectMapper) throws IOException {
        ...
        LOG.info("Loaded needs from file: " + filename);
    }
    ...
    private Need[] getNeedsArray(String containsText) { // if containsText == null, no filter
        ...
        for (Need need : needs.values()) {
            if (containsText == null || need.getDescription().contains(containsText)) {
    ...
    ** {@inheritDoc}
    ...
    public Need createNeed(Need need) throws IOException {
        synchronized(needs) {
            ...
            Need newNeed = new Need(nextId(),need.getName(), need.getDescription(), need.getQuantity(), need.getCost());
